...
export class Store {
  ...
  createBot(forcedName?: string): BotRecord {
    const name = forcedName?.trim() || pickBotName(this.bots.map((b) => b.name));
    const bot: BotRecord = {
      id: newId(),
      threadId: newId(),
      name,
  ...
  /** First-run seed: one bot so the app never opens empty — first bot is Junction. */
  seedIfEmpty() {
    if (this.bots.length) return;
    this.createBot("Junction");
  }
}
